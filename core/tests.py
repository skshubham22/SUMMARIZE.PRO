...
class SummarizationTests(TestCase):
    def test_smart_wiki_search_success(self):
        ...
        self.assertEqual(page.title.lower(), "python (programming language)")

    def test_smart_wiki_search_typo_correction(self):
        """Test that smart_wiki_search handles typos via its internal search logic."""
        # Custom search logic should handle close matches
        page = smart_wiki_search("Albrt Einstin")
        self.assertIsNotNone(page)
        self.assertIn("albert einstein", page.title.lower())
